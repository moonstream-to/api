...
ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS_RAW = os.environ.get(
    "ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS"
)
ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS = 2 * 1000 * ENGINE_DB_POOL_RECYCLE_SECONDS
try:
    if ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS_RAW is not None:
        ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS = int(
            ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS_RAW
        )
except:
    raise ValueError(
        f"ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS must be an integer: {ENGINE_DB_IDLE_SESSION_TIMEOUT_MILLISECONDS_RAW}"
    )
