...
class Game7Block(Base):  # type: ignore
...
class Game7Transaction(Base):  # type: ignore
    ...
    block_number = Column(
        BigInteger,
        ForeignKey("game7_blocks.block_number", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    from_address = Column(VARCHAR(256), index=True)
...
class Game7Label(Base):  # type: ignore
    __tablename__ = "game7_labels"

    __table_args__ = (
        Index(
            "ix_game7_labels_address_block_number",
            "address",
            "block_number",
            unique=False,
        ),
        Index(
            "ix_game7_labels_address_block_timestamp",
            "address",
            "block_timestamp",
            unique=False,
        ),
...
class Game7TestnetBlock(Base):  # type: ignore
...
class Game7TestnetTransaction(Base):  # type: ignore
...
class Game7TestnetLabel(Base):  # type: ignore
    __tablename__ = "game7_testnet_labels"

    __table_args__ = (
        Index(
            "ix_game7_testnet_labels_address_block_number",
            "address",
            "block_number",
            unique=False,
        ),
        Index(
            "ix_game7_testnet_labels_address_block_timestamp",
            "address",
            "block_timestamp",
            unique=False,
        ),
